...
function resolveGitAiBin(): string {
  const configured = process.env.GIT_AI_BIN?.trim();
  if (configured) return configured;

  const homeBin = join(homedir(), '.git-ai', 'bin', 'git-ai');
  return existsSync(homeBin) ? homeBin : 'git-ai';
}

const GIT_AI_BIN = resolveGitAiBin();
